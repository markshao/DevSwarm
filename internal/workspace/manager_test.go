...
func TestUpdateNodeStatus(t *testing.T) {
	...
	// 5. Verify status persisted by reloading
	wm2, err := NewManager(wm.RootPath)
	if err != nil {
		t.Fatalf("Failed to reload manager: %v", err)
	}
...
func TestUpdateNodeStatusNonExistent(t *testing.T) {
	...
	err := wm.UpdateNodeStatus("non-existent-node", types.StatusPushed)
	if err == nil {
...
func TestUpdateNodeStatusTransitions(t *testing.T) {
	...
	logicalBranch := "feature/transitions"

	// 1. Spawn Node
	err := wm.SpawnNode(nodeName, logicalBranch, "main", "Transitions test", true)
	if err != nil {
		t.Fatalf("SpawnNode failed: %v", err)
	}

	// 2. Test various status transitions
	transitions := []struct {
		from types.NodeStatus
		to   types.NodeStatus
	}{
		{types.StatusWorking, types.StatusReadyToPush},
		{types.StatusReadyToPush, types.StatusPushed},
		{types.StatusWorking, types.StatusFail},
	...
	for _, tr := range transitions {
		t.Run(string(tr.from)+"_to_"+string(tr.to), func(t *testing.T) {
			...
			err := wm.UpdateNodeStatus(nodeName, tr.to)
			if err != nil {
				t.Fatalf("UpdateNodeStatus failed: %v", err)
			}
			...
			if node.Status != tr.to {
				t.Errorf("Expected status to be %q, got %q", tr.to, node.Status)
			}
		})
	}
}
...
func TestUpdateNodeStatusPersistence(t *testing.T) {
	...
	logicalBranch := "feature/persist"

	// 1. Spawn Node
	err := wm.SpawnNode(nodeName, logicalBranch, "main", "Persist test", true)
	if err != nil {
		t.Fatalf("SpawnNode failed: %v", err)
	}

	// 2. Update status through all possible values
	...
	for _, status := range statuses {
		t.Run(string(status), func(t *testing.T) {
			...
			if loadedNode.Status != status {
				t.Errorf("Expected persisted status to be %q, got %q", status, loadedNode.Status)
			}
		})
	}
}
...
func TestSpawnNodeWithStatus(t *testing.T) {
	...
	nodeName := "test-node-spawn-status"
	...
	// 2. Verify node has StatusWorking by default
	node := wm.State.Nodes[nodeName]
	if node.Status != types.StatusWorking {
		t.Errorf("Expected default status to be WORKING, got %q", node.Status)
	}
	...
	if loadedNode.Status != types.StatusWorking {
		t.Errorf("Expected persisted default status to be WORKING, got %q", loadedNode.Status)
	}
}
