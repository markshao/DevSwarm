...
func TestPushBranch(t *testing.T) {
    // Create a temp directory to act as "remote" bare repo
    ...
    // Create a test branch to push
    cmd = exec.Command("git", "-C", localDir, "checkout", "-b", "test-branch")
    if output, err := cmd.CombinedOutput(); err != nil {
        t.Fatalf("failed to create test branch: %v, output: %s", err, output)
    }

    // Make a change on test-branch
    ...
    cmd = exec.Command("git", "-C", localDir, "add", ".")
    if err := cmd.Run(); err != nil {
        t.Fatalf("failed to git add on test branch")
    }

    cmd = exec.Command("git", "-C", localDir, "commit", "-m", "Add feature")
    if err := cmd.Run(); err != nil {
        t.Fatalf("failed to git commit on test branch")
    }

    // Set upstream and push using PushBranch
    // First set the upstream for the branch
    cmd = exec.Command("git", "-C", localDir, "push", "-u", "origin", "test-branch")
    if output, err := cmd.CombinedOutput(); err != nil {
        t.Fatalf("initial push failed: %v, output: %s", err, output)
    }

    // Make another change to test PushBranch
    featureFile2 := filepath.Join(localDir, "feature2.txt")
    if err := os.WriteFile(featureFile2, []byte("feature content 2"), 0644); err != nil {
        t.Fatalf("failed to write feature file 2: %v", err)
    }

    cmd = exec.Command("git", "-C", localDir, "add", ".")
    if err := cmd.Run(); err != nil {
        t.Fatalf("failed to git add on test branch 2")
    }

    cmd = exec.Command("git", "-C", localDir, "commit", "-m", "Add feature 2")
    if err := cmd.Run(); err != nil {
        t.Fatalf("failed to git commit on test branch 2")
    }

    // Test PushBranch (branch already has upstream set)
    if err := PushBranch(localDir, "test-branch"); err != nil {
        t.Fatalf("PushBranch failed: %v", err)
    }

    // Verify the branch exists in remote by checking remote refs
    cmd = exec.Command("git", "-C", localDir, "ls-remote", "origin", "refs/heads/test-branch")
    output, err := cmd.CombinedOutput()
    if err != nil {
        t.Fatalf("ls-remote failed: %v, output: %s", err, output)
    }

    // Check if the branch ref exists in remote output
    if len(output) == 0 {
        t.Errorf("test-branch was not pushed to remote")
    }
}
