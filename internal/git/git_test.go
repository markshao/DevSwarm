...
func TestGetChangedFiles(t *testing.T) {
    ...
    _ = exec.Command("git", "-C", repoPath, "add", "new.txt").Run()
    _ = exec.Command("git", "-C", repoPath, "commit", "-m", "Add new file").Run()

    // Get commit hashes
    fromHash, _ := exec.Command("git", "-C", repoPath, "rev-parse", "HEAD~1").Output()
    toHash, _ := exec.Command("git", "-C", repoPath, "rev-parse", "HEAD").Output()

    files, err := GetChangedFiles(repoPath, strings.TrimSpace(string(fromHash)), strings.TrimSpace(string(toHash)))
    if err != nil {
...
func TestMergeWorktree(t *testing.T) {
    ...
    _ = exec.Command("git", "-C", repoPath, "checkout", "-b", "feature").Run()
    featureFile := filepath.Join(repoPath, "feature.txt")
    if err := os.WriteFile(featureFile, []byte("feature content"), 0644); err != nil {
        t.Fatalf("failed to write feature file: %v", err)
    }
    _ = exec.Command("git", "-C", repoPath, "add", "feature.txt").Run()
    _ = exec.Command("git", "-C", repoPath, "commit", "-m", "Add feature").Run()

    // Checkout main and merge
    _ = exec.Command("git", "-C", repoPath, "checkout", "main").Run()

    // Test squash merge
    if err := MergeWorktree(repoPath, "feature", true); err != nil {
        t.Fatalf("MergeWorktree(squash) failed: %v", err)
    }

    // Verify file exists after squash
    if _, err := os.Stat(featureFile); os.IsNotExist(err) {
        t.Error("merged file not found after squash merge")
    }
}
...
func TestCommitWorktree(t *testing.T) {
    ...
    // Create a new file
    newFile := filepath.Join(repoPath, "commit-test.txt")
    if err := os.WriteFile(newFile, []byte("test content"), 0644); err != nil {
        t.Fatalf("failed to write file: %v", err)
    }

    if err := CommitWorktree(repoPath, "Test commit"); err != nil {
...
func TestPushBranch(t *testing.T) {
    ...
    remoteDir, err := os.MkdirTemp("", "orion-remote")
    if err != nil {
        t.Fatalf("failed to create temp dir: %v", err)
    }
    ...
    _ = exec.Command("git", "-C", repoPath, "remote", "add", "test-remote", remoteDir).Run()

    // Create a new branch with a commit
    _ = exec.Command("git", "-C", repoPath, "checkout", "-b", "test-push").Run()
    testFile := filepath.Join(repoPath, "test.txt")
    os.WriteFile(testFile, []byte("test"), 0644)
    _ = exec.Command("git", "-C", repoPath, "add", ".").Run()
    _ = exec.Command("git", "-C", repoPath, "commit", "-m", "Test commit").Run()

    // Push should succeed (no auth needed for local)
    // Note: This test may fail in environments without proper git setup
    // It's kept for completeness but may need to be skipped in CI
    t.Skip("Skipping push test - requires proper git remote setup")
}
...
func TestBranchExists(t *testing.T) {
    ...
    exists, err = BranchExists(repoPath, "nonexistent")
    if err != nil {
        t.Fatalf("BranchExists failed: %v", err)
    }
    if exists {
...
func TestDeleteBranch(t *testing.T) {
    ...
    // Create a branch
    _ = exec.Command("git", "-C", repoPath, "branch", "to-delete", "main").Run()

    if err := DeleteBranch(repoPath, "to-delete"); err != nil {
        t.Fatalf("DeleteBranch failed: %v", err)
    }

    exists, _ := BranchExists(repoPath, "to-delete")
    if exists {
