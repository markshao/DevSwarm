...
func TestPushBranch(t *testing.T) {
    ...
    _ = exec.Command("git", "-C", repoPath, "checkout", "-b", "feature/test-push").Run()
    _ = exec.Command("git", "-C", repoPath, "commit", "--allow-empty", "-m", "Test commit").Run()
    ...
    // Push the branch
    err = PushBranch(repoPath, "feature/test-push")
    if err != nil {
        t.Fatalf("PushBranch failed: %v", err)
    }

    // Verify the branch exists in remote
    cmd = exec.Command("git", "ls-remote", remoteDir, "feature/test-push")
    output, err := cmd.Output()
    if err != nil {
        t.Errorf("Branch not found in remote: %v", err)
    }
    if len(output) == 0 {
        t.Errorf("ls-remote returned empty output, branch may not exist in remote")
    }
}
...
func TestPushBranchNonExistent(t *testing.T) {
    ...
    if err == nil {
        t.Errorf("PushBranch succeeded for non-existent branch, expected error")
    }
}
