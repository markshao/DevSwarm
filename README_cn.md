...
**Orion** 是一个专为 AI Native 开发时代设计的开发环境管理工具，旨在促进人类开发者与 AI Agent 之间的高效并发协作。

在 AI Coding 时代，传统的 Git 工作流难以支持在同一个逻辑分支上进行多任务并发。Orion 通过将 Git 分支虚拟化为 **Nodes（节点）** 来解决这个问题，每个节点都提供了一个隔离的环境（Worktree + Tmux Session），使得人类或 Agent 可以在互不干扰的情况下独立工作。
...
- **影子分支 (Shadow Branching)**：自动管理 `影子分支` (例如 `orion/login-test/feature/login`)，允许在同一个逻辑功能上并行工作。
...
```bash
curl -sL https://raw.githubusercontent.com/markshao/Orion/main/install.sh | bash
```

该脚本将执行以下操作：

1. 检测您的操作系统和架构。
2. 从 [GitHub Releases](https://github.com/markshao/Orion/releases) 下载最新的二进制文件。
3. 安装 `orion` 到 `/usr/local/bin`。
4. 为您的 Shell (Zsh/Bash) 配置自动补全。

## 🎮 演练场 (Playground)

Orion 附带了一个自动化的 **端到端 (E2E) 测试脚本**，模拟完整的用户工作流。这是在不污染环境的情况下了解 Orion 工作原理的最佳方式。
...
为 Git 仓库创建一个 Orion 工作区。

```bash
# 自动创建 'repo_workspace' 目录 (例如 Orion_workspace)
orion init https://github.com/markshao/Orion.git

# 或者指定自定义目录名称
orion init https://github.com/markshao/Orion.git my_custom_workspace
```
...
orion spawn feature/login login-dev --base main --purpose coding
```

#### 影子模式 (Shadow Mode, --shadow)

基于目标分支创建一个临时的影子分支 (`orion-shadow/...`)。最适合代码审查、测试或实验性更改，而不会污染主分支。

```bash
# 基于 'feature/login' 创建一个审查节点，而不检出该分支本身
orion spawn feature/login login-review --shadow --purpose review
```
...
orion merge login-dev

# 压缩合并并自动移除节点
orion merge login-dev --cleanup
```
...
Orion 提供了与现代 IDE 的深度集成，以增强您的工作流。

### 1. 工作区集成 (推荐)

Orion 会自动生成并维护一个标准的 `.code-workspace` 文件。导入此文件允许您在单个 IDE 窗口中同时编辑多个节点的文件，并获得完整的语言服务器支持。
...
Orion 可以根据您当前编辑的文件，自动将您的 IDE 终端连接到正确的 Node Tmux 会话。
...
```json
  ...
  "terminal.integrated.defaultProfile.osx": "orion-tmux"
```
...
- 当您打开终端 (`Ctrl + ~`) 时，Orion 会检测当前活动文件 (通过 `CURRENT_FILE` 传递) 是否属于特定的 **Node**。
